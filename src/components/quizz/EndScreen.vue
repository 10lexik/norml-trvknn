...
<template>
  <div class="screen end-screen">
    <div class="score-badge-wrapper">
      <div v-if="medal" class="crown-frame" :class="medal">
        <svg viewBox="0 0 182.86 155" xmlns="http://www.w3.org/2000/svg">
          <g id="crown-paths" :style="{ fill: crownColor }">
            <path
            ...
            <path d="M45.8,140s10.37,12.68,27.93,11.88l-.21-3.34s-12.97,3.06-26.53-9.47l-1.19.93Z" />
            <path d="M136.74,140s-10.37,12.68-27.93,11.88l.21-3.34s12.97,3.06,26.53-9.47l1.19.93Z" />
          </g>
        </svg>
      </div>
      <div class="score-numeric">
        ...
        <div class="score-row">
          ...
          <span class="total-inline">/ {{ total }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="!isSaved" ref="formRef" class="save-form">
      ...
      <div class="form-group primary-group">
        <div class="form-row" :class="{ 'has-error': nameError }">
          ...
          <input type="text" :value="nameModel" @input="onNameInput" @focus="scrollToForm"
            :placeholder="t('end.placeholder_name')" maxlength="15" />
        </div>
        ...
        <div class="form-row" :class="{ 'has-error': emailError }">
          ...
          <input type="email" :value="emailModel" @input="onEmailInput" @focus="scrollToForm"
            :placeholder="t('end.placeholder_email')" />
        </div>

        <div class="consent-row">
          <label class="consent-label">
            <input type="checkbox" :checked="consentModel" @change="onConsentChange" />
      ...
      <div class="form-group optional-group">
        <div class="form-row">
          <span class="prefix-icon">#</span>
          <input type="text" :value="memberIdModel" @input="onMemberIdInput" @focus="scrollToForm"
            :placeholder="t('end.placeholder_id')" />
        </div>

        <div class="social-section" v-if="socialNetworks.length">
          <div class="social-bar">
            <button v-for="net in socialNetworks" :key="net.id" class="icon-btn"
              :class="{ active: visibleNetworks.includes(net.id) || socialsModel[net.id] }"
              @click="emit('toggleNetwork', net.id)" :title="net.label">
              <span v-html="net.icon"></span>
            </button>
          </div>
          <transition-group name="slide">
            <div v-for="net in socialNetworks" :key="net.id" class="form-row social-row"
              v-show="visibleNetworks.includes(net.id) || socialsModel[net.id]">
              <span class="prefix-icon social-icon" v-html="net.icon"></span>
              <input type="text" :value="socialsModel[net.id]" @input="(e) => onSocialInput(net.id, e)"
                @focus="scrollToForm" :placeholder="net.label" />
      ...
      <i18n-t keypath="end.rgpd_global_disclaimer" tag="p" class="legal-disclaimer">
      ...
      <div class="actions-row">
        <button class="btn-primary btn-action-trigger" @click="emit('save')"
                ...
                <td class="name">
                  ...
                  <div class="social-icons" v-if="entry.socials && socialNetworks.length">
                    <template v-for="net in socialNetworks" :key="net.id">
                      <a v-if="entry.socials[net.id]" :href="entry.socials[net.id]" target="_blank" class="s-lnk"
                        v-html="net.icon"></a>
                    </template>
                  </div>
                </td>
    ...
    <div v-if="showShareModal" class="share-modal-overlay" @click.self="emit('closeModal')">
      <div class="share-modal-content">
        ...
        <div class="share-buttons-row">
          <button v-for="net in socialNetworks" :key="net.id" class="btn-network-circle"
            :style="{ backgroundColor: net.color }" @click="emit('download', net)" :title="net.label">
